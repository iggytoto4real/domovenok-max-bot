...
- **prod** — запуск внутри MAX:
  ...
  - покупка нового домового отправляется на `POST /api/pets` (имя и тип); backend-core списывает фиксированную цену в денюжках и создаёт питомца, ответ маппится в `PetItem` и добавляется в список;
